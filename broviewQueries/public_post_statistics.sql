CREATE VIEW public_post_statistics as
SELECT p.class_id as c_id, avg(p.wordcount) as avg_wc, min(p.wordcount) as min_wc, max(p.wordcount) as max_wc,
       ...
       count(*) as num_of_public_posts
FROM
(SELECT class_id, wordcount, bro_sentiment FROM posts where private=FALSE and wordcount>0) p
GROUP BY c_id;
